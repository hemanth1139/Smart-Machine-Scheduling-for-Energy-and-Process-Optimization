...
class SchedulingExporter:
    ...
    def export_schedules_and_kpis(
        ...
        comp_df = pd.DataFrame([
            ...
            {"Metric": "Makespan (min)", "FCFS_Baseline": fcfs_kpis.get("Makespan_min", 0), "CP_SAT_Optimized": opt_kpis.get("Makespan_min", 0), "Improvement": f"{makespan_diff} min reduction"},
            {"Metric": "On-Time Completion Rate (%)", "FCFS_Baseline": f"{fcfs_kpis.get('On_Time_Completion_%', 0)}%", "CP_SAT_Optimized": f"{opt_kpis.get('On_Time_Completion_%', 0)}%", "Improvement": f"+{ontime_diff:.1f}% improvement"},
            {"Metric": "Machine Utilization (%)", "FCFS_Baseline": f"{fcfs_kpis.get('Machine_Utilization_%', 0)}%", "CP_SAT_Optimized": f"{opt_kpis.get('Machine_Utilization_%', 0)}%", "Improvement": f"+{util_diff:.1f}% improvement"},
        ])
    ...
    def generate_markdown_report(
        ...
        lines = [
            ...
            f"| **Makespan (Hours)** | `{fcfs_kpis.get('Makespan_hours', 0)}` hrs | `{opt_kpis.get('Makespan_hours', 0)}` hrs | **{fcfs_kpis.get('Makespan_min', 0) - opt_kpis.get('Makespan_min', 0)} min reduction** |",
            f"| **On-Time Job Completion Rate (%)** | `{fcfs_kpis.get('On_Time_Completion_%', 0)}%` | `{opt_kpis.get('On_Time_Completion_%', 0)}%` | **+{opt_kpis.get('On_Time_Completion_%', 0) - fcfs_kpis.get('On_Time_Completion_%', 0):.1f}%** |",
            f"| **Machine Utilization (%)** | `{fcfs_kpis.get('Machine_Utilization_%', 0)}%` | `{opt_kpis.get('Machine_Utilization_%', 0)}%` | **+{opt_kpis.get('Machine_Utilization_%', 0) - fcfs_kpis.get('Machine_Utilization_%', 0):.1f}%** |",
